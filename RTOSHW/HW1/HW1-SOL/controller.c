...
void * control_loop(void * par) {
	...
	// Apriamo la coda per mandare gli hearthbeat ogni due cicli in sola scrittura
	// si suppone che parta prima la replica quindi | O_CREAT non è necessario
	mqd_t wd_qd;
		if ((wd_qd = mq_open (WDOG_QUEUE_NAME, O_WRONLY | O_CREAT , QUEUE_PERMISSIONS,&attr)) == -1) {
			perror ("control loop: mq_open (wd_qd)");
			exit (1);
		}	
	...
	int nciclo = 0; //ogni due cicli dovrò mandare l'heartbeat
	...
	while (keep_on_running)
	{
		...
		if(nciclo==2){
			nciclo=0; //azzero e riconterò altri 2 cicli
			//in message devo scrivere l'ultima reference
			sprintf(message, "%d", reference);
			if(mq_send(wd_qd,message,strlen(message)+1,0)==-1){
				perror("heartbeat send wd_qd");
        		exit (1);
			}
		}
	}
