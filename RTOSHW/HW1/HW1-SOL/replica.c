...
static struct shared_int last_original_reference;
...
void* watchdog(void* par){
		...
		// Apriamo la coda watchdog per gli heartbeat da ricevere dal controller originale 
		mqd_t wd_qd;
		...
		while(keep_on_running){
			...
			if(mq_timedreceive(wd_qd,message,MAX_MSG_SIZE,NULL,&timeout)==-1){ //se non ho ricevuto l'heartbeat entro il timeout
...
int main(void)
{
	...
	last_original_reference.value=110;
